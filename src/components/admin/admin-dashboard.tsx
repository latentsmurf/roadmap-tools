...
import { Search, Plus, ExternalLink, Settings, Shield } from "lucide-react"
...
export function AdminDashboard({ roadmaps, userEmail, userRole }: AdminDashboardProps) {
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-10">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <PermissionGuard permission="roadmap:create">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredRoadmaps.length === 0 && (
          <div className="col-span-full py-20 text-center bg-muted/20 rounded-3xl border-2 border-dashed border-border/50">
            <p className="text-muted-foreground">
              No roadmaps found. Create your first one to get started!
            </p>
          </div>
        )}
        {filteredRoadmaps.map((roadmap) => (
          <Card
            key={roadmap.id}
            className="group overflow-hidden rounded-3xl border border-border/40 hover:border-primary/50 transition-all hover:shadow-xl hover:-translate-y-1"
          >
            <CardHeader className="bg-muted/30 pb-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" asChild>
                    <Link
                      href={`/r/${roadmap.workspaceId || "default"}/${roadmap.slug}`}
                      target="_blank"
                    >
              ...
              <div className="flex gap-2">
                <PermissionGuard permission="item:create">
                  <Button variant="secondary" className="flex-1 rounded-xl font-semibold" asChild>
                    <Link href={`/admin/items/new?roadmapId=${roadmap.id}`}>+ Item</Link>
                  </Button>
                </PermissionGuard>
